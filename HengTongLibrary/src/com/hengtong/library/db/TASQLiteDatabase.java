 ...
 */
package com.hengtong.library.db;
...
import com.hengtong.library.db.enty.TAArrayList;
import com.hengtong.library.db.enty.TADBMasterEntity;
import com.hengtong.library.db.enty.TAHashMap;
import com.hengtong.library.db.enty.TAMapArrayList;
import com.hengtong.library.db.exception.TADBException;
import com.hengtong.library.db.exception.TADBNotOpenException;
import com.hengtong.library.db.util.TADBUtils;
import com.hengtong.library.db.util.TASqlBuilderFactory;
import com.hengtong.library.db.util.sql.TASqlBuilder;
...
public class TASQLiteDatabase {
	...
	private final static String TAG = TASQLiteDatabase.class.getSimpleName();
	// 当前SQL指令
	...
	public TASQLiteDatabase(Context context) {
		...
		this.mDatabaseHelper = new TADBHelper(context, params.getDbName(), null, params.getDbVersion());
	}
	 ...
	 * @return
	 */
	public SQLiteDatabase openDatabase(TADBUpdateListener dbUpdateListener, Boolean isWrite) {
	...
	public ArrayList<TAHashMap<String>> query(String sql, String[] selectionArgs) {
		...
		if (testSQLiteDatabase()) {
			...
			if (queryCursor != null) {
				...
				LogControl.e(TAG, "执行" + sql + "错误");
			}
		} else {
			LogControl.e(TAG, "数据库未打开！");
		}
		return null;
	}
	...
	public <T> List<T> query(Class<?> clazz, boolean distinct, String where, String groupBy, String having, String orderBy, String limit) {

		if (testSQLiteDatabase()) {
			List<T> list = null;
			TASqlBuilder getSqlBuilder = TASqlBuilderFactory.getInstance().getSqlBuilder(TASqlBuilderFactory.SELECT);
			...
			try {
				...
				LogControl.i(TAG, "执行" + sqlString);
				free();
				this.queryCursor = mSQLiteDatabase.rawQuery(sqlString, null);
				list = (List<T>) TADBUtils.getListEntity(clazz, this.queryCursor);
	 ...
	 * @return
	 */
	public ArrayList<TAHashMap<String>> query(String table, String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy) {
		if (testSQLiteDatabase()) {
			this.queryCursor = mSQLiteDatabase.query(table, columns, selection, selectionArgs, groupBy, having, orderBy);
			if (queryCursor != null) {
				return getQueryCursorData();
			} else {
				LogControl.e(TAG, "查询" + table + "错误");
			}
		} else {
			LogControl.e(TAG, "数据库未打开！");
		}
	 ...
	 * @return
	 */
	public ArrayList<TAHashMap<String>> query(String table, boolean distinct, String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy, String limit) {
		if (testSQLiteDatabase()) {
			free();
			this.queryCursor = mSQLiteDatabase.query(distinct, table, columns, selection, selectionArgs, groupBy, having, orderBy, limit);
			if (queryCursor != null) {
				return getQueryCursorData();
			} else {
				LogControl.e(TAG, "查询" + table + "错误");
	 ...
	 * @return
	 */
	public ArrayList<TAHashMap<String>> query(String table, String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy, String limit) {

		if (testSQLiteDatabase()) {
			free();
			this.queryCursor = mSQLiteDatabase.query(table, columns, selection, selectionArgs, groupBy, having, orderBy, limit);
			if (queryCursor != null) {
				return getQueryCursorData();
			} else {
				LogControl.e(TAG, "查询" + table + "错误");
	 ...
	 * @return
	 */
	public ArrayList<TAHashMap<String>> queryWithFactory(CursorFactory cursorFactory, boolean distinct, String table, String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy, String limit) {
		if (testSQLiteDatabase()) {
			free();
			this.queryCursor = mSQLiteDatabase.queryWithFactory(cursorFactory, distinct, table, columns, selection, selectionArgs, groupBy, having, orderBy, limit);
			if (queryCursor != null) {
				return getQueryCursorData();
			} else {
				LogControl.e(TAG, "查询" + table + "错误");
	...
	public void execute(String sql, String[] bindArgs) throws TADBNotOpenException {
		LogControl.i(TAG, "准备执行SQL[" + sql + "]语句");
		if (testSQLiteDatabase()) {
			if (sql != null && !sql.equalsIgnoreCase("")) {
				this.queryStr = sql;
				if (bindArgs != null) {
	...
	public TAMapArrayList<String> getQueryCursorData() {
		...
		if (queryCursor != null) {
			try {
				...
				e.printStackTrace();
				LogControl.e(TAG, "当前数据集获取失败！");
			}
		} else {
			LogControl.e(TAG, "当前数据集不存在！");
		}
		return arrayList;
	}
	...
	public Boolean insert(Object entity, TAArrayList updateFields) {
		...
		TASqlBuilder getSqlBuilder = TASqlBuilderFactory.getInstance().getSqlBuilder(TASqlBuilderFactory.INSERT);
	...
	public Boolean update(Object entity, String where) {
		if (testSQLiteDatabase()) {
			TASqlBuilder getSqlBuilder = TASqlBuilderFactory.getInstance().getSqlBuilder(TASqlBuilderFactory.UPDATE);
			getSqlBuilder.setEntity(entity);
			getSqlBuilder.setCondition(false, where, null, null, null, null);
			return execute(getSqlBuilder);
		} else {
