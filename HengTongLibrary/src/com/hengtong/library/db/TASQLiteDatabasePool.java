...
import com.hengtong.library.db.TASQLiteDatabase.TADBParams;
import com.hengtong.library.db.TASQLiteDatabase.TADBUpdateListener;
...
public class TASQLiteDatabasePool {
	...
	private final static String TAG = TASQLiteDatabasePool.class.getSimpleName();
	...
	private void createSQLiteDatabase(int numSQLiteDatabase) {
		...
		for (int x = 0; x < numSQLiteDatabase; x++) {
			...
			try {
				...
				LogControl.i(TAG, " 创建数据库连接失败！ " + e.getMessage());
			}
			LogControl.i(TAG, "数据库连接己创建 ......");
		}
	}
	...
	/**
	 * 本函数从连接池向量 pSQLiteDatabases 中返回一个可用的的数据库连接，如果 当前没有可用的数据库连接，本函数则根据 incrementalSQLiteDatabase 设置 的值创建几个数据库连接，并放入连接池中。 如果创建后，所有的连接仍都在使用中，则返回 null
	 * @return 返回一个可用的数据库连接
	 */
	...
	public void releaseSQLiteDatabase(TASQLiteDatabase sqLiteDatabase) {
		...
		if (pSQLiteDatabases == null) {
			LogControl.d(TAG, " 连接池不存在，无法返回此连接到连接池中 !");
			return;
		}
		PooledSQLiteDatabase pSqLiteDatabase = null;
