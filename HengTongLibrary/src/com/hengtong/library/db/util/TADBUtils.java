 ...
 */
package com.hengtong.library.db.util;
...
import com.hengtong.library.db.annotation.TAColumn;
import com.hengtong.library.db.annotation.TAPrimaryKey;
import com.hengtong.library.db.annotation.TATransient;
import com.hengtong.library.db.enty.TAHashMap;
import com.hengtong.library.db.enty.TAPKProperyEntity;
import com.hengtong.library.db.enty.TAPropertyEntity;
import com.hengtong.library.db.enty.TATableInfoEntity;
...
public class TADBUtils {
	...
	public static String getTableName(Class<?> clazz) {
		...
		String dddString = clazz.getSimpleName().toLowerCase();
		return dddString;
	}
			...
			for (Field field : fields) {
				if (!TADBUtils.isTransient(field)) {
					if (TADBUtils.isBaseDateType(field)) {
						...
						property.setColumnName(TADBUtils.getColumnByField(field));
						property.setName(field.getName());
						property.setType(field.getType());
						property.setDefaultValue(TADBUtils.getPropertyDefaultValue(field));
						plist.add(property);
					}
				}
			}
	...
	public static String creatTableSql(Class<?> clazz) throws TADBException {
		TATableInfoEntity tableInfoEntity = TATableInfofactory.getInstance().getTableInfoEntity(clazz);
		...
		Collection<TAPropertyEntity> propertys = tableInfoEntity.getPropertieArrayList();
