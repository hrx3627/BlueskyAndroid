 ...
 */
package com.hengtong.library.db.util.sql;
...
import com.hengtong.library.db.annotation.TAPrimaryKey;
...
public class TAUpdateSqlBuilder extends TASqlBuilder {
	...
	public String buildSql() throws TADBException, IllegalArgumentException, IllegalAccessException {
		...
		for (int i = 0; i < needUpdate.size(); i++) {
			...
			stringBuilder.append(nameValuePair.getName()).append(" = ").append(StringUtils.isNumeric(nameValuePair.getValue().toString()) ? nameValuePair.getValue() : "'" + nameValuePair.getValue() + "'");
			if (i + 1 < needUpdate.size()) {
				stringBuilder.append(", ");
			}
		}
		if (!StringUtils.isNullOrEmpty(this.where)) {
	 ...
	 */
	public TAArrayList buildWhere(Object entity) throws IllegalArgumentException, IllegalAccessException, TADBException {
		...
		for (Field field : fields) {
			...
			if (!TADBUtils.isTransient(field)) {
				if (TADBUtils.isBaseDateType(field)) {
					Annotation annotation = field.getAnnotation(TAPrimaryKey.class);
					if (annotation != null) {
						String columnName = TADBUtils.getColumnByField(field);
						whereArrayList.add((columnName != null && !columnName.equals("")) ? columnName : field.getName(), field.get(entity).toString());
					}
	 ...
	 * @throws IllegalAccessException
	 */
	public static TAArrayList getFieldsAndValue(Object entity) throws TADBException, IllegalArgumentException, IllegalAccessException {
		// TODO Auto-generated method stub
			...
			if (!TADBUtils.isTransient(field)) {
				if (TADBUtils.isBaseDateType(field)) {
					...
					if (annotation == null || !annotation.autoIncrement()) {
						String columnName = TADBUtils.getColumnByField(field);
						field.setAccessible(true);
						arrayList.add((columnName != null && !columnName.equals("")) ? columnName : field.getName(), field.get(entity) == null ? null : field.get(entity).toString());
					}
				}
			}
		}
